...
7. Symlink semantics are explicit: `target` in the JSON output is the lexical absolute path selected by the user, while existing recorded directories are canonicalized and checked with segment-safe physical containment. A missing recorded directory falls back to segment-safe lexical containment against both the selected alias and its canonical target. Claude Code, Codex, Copilot, and OpenCode records can therefore match either alias form without admitting a symlink escape. Gemini's Capsule format has `projectHash`, not a recoverable `cwd`; accept only the exact SHA-256 hash of the selected alias or its canonical target, and exclude descendant Gemini launches.
...
3. Run `node "<skill-dir>/scripts/scrub_export_paths.mjs" --root "$TARGET_FOLDER" --export-dir "$TARGET_FOLDER/session-export"`. It parses JSON and JSONL, replaces both lexical and canonical target-folder paths in decoded string values and object keys (including alternate separators and Windows case variants), validates all staged results, and writes nothing if any export is malformed or retains a path variant.
